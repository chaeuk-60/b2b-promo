...
- **폰트**: 도트/픽셀 폰트 `Galmuri11`(한글 지원, SIL OFL) 1종만 전역 사용. `npm galmuri` 패키지를 jsDelivr CDN(`https://cdn.jsdelivr.net/npm/galmuri/dist/galmuri.css`)으로 `@import` 로드(사용자 확인: 처음엔 모노스페이스 폴백만 쓰다가 실제 도트 폰트로 교체). 폴백: `'Courier New', ui-monospace, Consolas, monospace`.
...
모든 버튼은 모양(픽셀 노치)·높이·패딩이 동일하고, 배경색으로만 종류를 구분한다(`min-height` 고정 + `display: inline-flex; align-items: center; justify-content: center`로 글자 길이/폰트 크기와 무관하게 높이를 통일). `min-width: 88px`도 함께 고정해서 "찜"처럼 짧은 라벨과 "쓰다듬기"처럼 긴 라벨이 섞여도 폭이 들쭉날쭉하지 않게 하고, 그보다 긴 글자만 자연스럽게 늘어나게 한다(사용자 확인). 나란히 놓인 버튼 사이에는 `.pixel-btn + .pixel-btn { margin-left: var(--space-2); }`로 간격을 준다(버튼끼리 붙어 보이지 않게 - 단, flex `gap`으로 간격을 주는 줄에서는 이 마진을 0으로 되돌린다).
...
- **찜 버튼**: 흰 배경(찜 안 함)/`--accent-pink` 배경(찜 함) + 하트 아이콘(♥/♡), 눌렀을 때 살짝 튀는 애니메이션(`transform: scale(1.2)`, 150ms). 모양·높이는 다른 버튼과 동일. 목록/상세 모두 신청 상태 버튼 옆(오른쪽)에 나란히 배치한다.
- **로그인/회원가입 버튼**: 한 줄에 좌우로 배치하되, 라벨/위치는 현재 로그인/회원가입 모드와 무관하게 항상 "로그인"이 왼쪽, "회원가입"이 오른쪽으로 고정된다. 현재 모드에 해당하는 쪽만 주요 버튼(초록)이고 반대쪽은 그 모드로 전환하는 보조 버튼(흰 배경)이다.
...
## 6-2. 목록/상세 제목, 상단 탭, 안내 각주

- **제목 하이라이트**: 프로모션 목록·상세·나의 신청 목록의 카드 제목과 펫 이름은 밑줄 대신 옅은 노란 형광펜(하이라이터) 효과를 준다 - `background: linear-gradient(180deg, transparent 55%, #fff3a0 55%)`를 제목 텍스트에만 걸어(이모지는 제외) 글자 아래쪽 절반에 마커로 칠한 느낌을 낸다(사용자 확인). 목록 카드에는 6-1의 제목표시줄(색깔 바)을 쓰지 않고, 펫 팝업/프로모션 상세 화면에만 제목표시줄을 쓴다.
- **상단 탭**: 네비게이션 링크도 버튼과 같은 `.pixel-btn` 모양을 그대로 쓰고, 현재 페이지에 해당하는 탭만 `--brand-blue` 배경 + 흰 글자로 강조한다(react-router `NavLink`로 `isActive` 판별).
- **안내 각주**: "취소는 담당자에게 연락 주세요"/"담당자에게 연락 주세요" 같은 보조 안내 문구는 본문보다 작은 글자(12px, `--muted` 색)로, 앞에 당구장 표시(※)를 붙여 각주처럼 보이게 한다(사용자 확인).

## 7. 배경/장식

- 페이지 전체 배경: 부드러운 CSS 그라데이션 대신, 격자에 칸마다 색을 찍고 밝기를 살짝 흔든(디더링) Y2K/레트로퓨처 픽셀 그라데이션 SVG(`frontend/public/images/app-bg.svg`, 시드 고정 스크립트로 생성)를 `background: url(...) center / cover fixed`로 깐다. 색 흐름은 민트/시안(위) → 옐로/골드(중간) → 핑크/마젠타(아래)의 대각선 그라데이션(사용자 제공 레퍼런스 이미지 기준). 칸이 화면 비율에 따라 직사각형으로 찌그러지지 않도록 SVG `preserveAspectRatio`는 기본값을 유지한다.
- 펫 화면에서는 배경을 펫의 mood(`11-pet-state-diagram.md`)에 맞춰 톤만 미세하게 바꿔도 됨(예: 행복=하늘색 쨍하게, 삐짐=톤 다운). 새 애셋 제작 없이 CSS 필터/배경색 정도로 그친다. 예외로 "오늘의 운세"를 볼 때는 펫을 가운데로 멈춰 세우고 어두운 별밤 스포트라이트 배경(역시 하드 스톱 그라데이션, blur 없음)으로 전환한다.
...
**스킵한 것**: 다크모드, 애니메이션 라이브러리 도입 — 필요해지면 그때 추가.
