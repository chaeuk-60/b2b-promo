...
- `6-project-principle.md` 4장 지침대로 확률/분기 로직만 테스트한다: 성장 전이 50%, 특식 요청 대상 선정 70% 가중치, 자발적 급여 50%/50%(진화/반짝이), 성체 선물 5%+3일 쿨다운, 사망 vs 성체순환 우선순위, mood 로그인별 재계산(2회차 70% 평범/3회차 30% 행복). `Math.random`을 주입/모킹해 분기별 결과만 검증(통계적 반복 검증은 만들지 않음).
- 선행: BE-6, BE-7
- 완료 조건:
