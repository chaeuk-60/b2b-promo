...
- `backend/src/services/pet.service.js`에 `getPet`, `nameOwnPet`(이름 짓기/재설정), `resolveMoodOnLogin`(자정 기준 KST로 날짜가 바뀌면 그날 접속 횟수 카운트를 초기화하고, 매 로그인마다 도메인 정의서 5.1절 규칙대로 mood/eggState를 재계산: 1회차 균등 랜덤 → 2회차부터 70% 평범/30% 재추첨 → 3회차부터 30% 확률로 행복 덮어씀. 단 이미 오늘 "행복"/"반짝이"/"무지개"가 됐으면 재계산하지 않고 그대로 유지. lazy 처리, 별도 배치 없음) 함수를 작성한다.
...
- 완료 조건:
  ...
  - [ ] 오늘 하루 중 mood가 "행복"/"반짝이"/"무지개"가 된 적이 있으면, 이후 같은 날 재로그인해도 재계산되지 않고 그 상태가 유지된다.

### BE-6. 펫 행동 처리 (목욕/밥/특식주기/쓰다듬기, 성장 전이)
- `pet.service.js`에 `applyAction(action)`(목욕/밥/쓰다듬기 - 도메인 정의서 5.1 표대로 mood/eggState 개선), `feedSpecialFood(promotionId)`(보유 특식 급여: 요청중이면 반짝이, 자발적이면 50% 진화/50% 무지개), `checkStageTransition`(알→새끼, 새끼→성체, activityCount 조건 충족 시 50% 판정)을 구현한다.
...
- 완료 조건:
  ...
  - [ ] 특식 요청 상태에서 대상 특식 급여 시 반짝이로 변하고, 자발적 급여 시 50% 확률로 진화, 50% 확률로 무지개로 변한다(둘 중 하나는 항상 발생).
...
- `6-project-principle.md` 4장 지침대로 확률/분기 로직만 테스트한다: 성장 전이 50%, 특식 요청 대상 선정 70% 가중치, 자발적 급여 50%/50%(진화/무지개), 성체 선물 5%+3일 쿨다운, 사망 vs 성체순환 우선순위, mood 로그인별 재계산(2회차 70% 평범/3회차 30% 행복). `Math.random`을 주입/모킹해 분기별 결과만 검증(통계적 반복 검증은 만들지 않음).
- 선행: BE-6, BE-7
- 완료 조건:
