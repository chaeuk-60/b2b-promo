...
- 선행: DB-1, BE-1
- 완료 조건:
  - [x] 회원가입 API 호출 시 users 레코드와 pets 레코드(알 상태)가 함께 생성된다.
  - [x] 로그인 성공 시 액세스 토큰(Body/헤더)과 리프레시 토큰(httpOnly 쿠키)이 발급되고, `refresh_tokens`에 해시 1건이 저장된다.
  - [x] 로그아웃 호출 시 해당 `refresh_tokens` 행이 삭제되어 같은 리프레시 토큰으로 재발급이 실패한다.
  - [x] 액세스 토큰 없이 보호된 API 호출 시 401이 반환되고, 만료된 액세스 토큰은 리프레시 엔드포인트로 재발급된다.
...
- 선행: BE-2
- 완료 조건:
  - [x] 로그인 API 호출 시 그날 몇 번째 접속인지에 따라 mood(또는 eggState)가 도메인 정의서 5.1절 규칙대로 재계산된다(1회차 랜덤, 2회차부터 대체로 평범).
  - [x] 이름 짓기 API로 이름을 설정/변경할 수 있고, 건너뛰면 기본 이름이 표시된다.
  - [x] 오늘 하루 중 mood가 "행복"/"무지개"/"반짝이"가 된 적이 있으면, 이후 같은 날 재로그인해도 재계산되지 않고 그 상태가 유지된다.

### BE-6. 펫 행동 처리 (목욕/밥/특식주기/쓰다듬기, 성장 전이)
- `pet.service.js`에 `applyAction(action)`(목욕/밥/쓰다듬기 - 도메인 정의서 5.1 표대로 mood/eggState 개선), `feedSpecialFood(promotionId)`(보유 특식 급여: 요청중이면 무지개, 자발적이면 50% 진화/50% 반짝이), `checkStageTransition`(알→새끼, 새끼→성체, activityCount 조건 충족 시 50% 판정)을 구현한다.
...
- 완료 조건:
  - [x] 알 단계에서 목욕/밥/특식주기/쓰다듬기 호출 시 도메인 정의서 5.1 표대로 eggState와 activityCount가 변한다.
  - [x] 새끼/성체 단계에서 동일 행동이 mood를 표대로 변화시키고 "밥"은 특식 효과가 전혀 없다.
  - [x] activityCount가 전이 조건(2회/누적5회)을 충족하면 50% 확률로 다음 단계로 성장하고, 실패해도 activityCount는 유지된다.
  - [x] 특식 요청 상태에서 대상 특식 급여 시 무지개로 변하고, 자발적 급여 시 50% 확률로 진화, 50% 확률로 반짝이로 변한다(둘 중 하나는 항상 발생).
...
- 선행: BE-5
- 완료 조건:
  - [x] 7일 이상 미접속 후 로그인하면 stage가 "묘비"이고, 그 다음 로그인에서 새 알로 교체(activityCount=0, name=null)된다.
  - [x] 사망 조건과 성체 순환 조건이 동시에 충족되면 사망이 우선 적용된다.
  - [x] 성체 순환(2주 경과) 시 확률과 무관하게 선물이 100% 지급되고 새 알로 교체된다.
  - [x] 성체 접속 시 5% 확률로만 선물이 지급되고, 마지막 지급 후 3일 이내면 지급되지 않는다.
...
- `6-project-principle.md` 4장 지침대로 확률/분기 로직만 테스트한다: 성장 전이 50%, 특식 요청 대상 선정 70% 가중치, 자발적 급여 50%/50%(진화/반짝이), 성체 선물 5%+3일 쿨다운, 사망 vs 성체순환 우선순위, mood 로그인별 재계산(2회차 70% 평범/3회차 70% 행복), 특식 요청 후보 프로모션 없을 때 제외 처리. `Math.random`을 주입/모킹해 분기별 결과만 검증(통계적 반복 검증은 만들지 않음).
- 선행: BE-6, BE-7
- 완료 조건:
  - [x] 오늘의 운세 API가 새끼/성체에서만 동작하고, 하루 내 재호출 시 같은 결과를 반환한다.
  - [x] 위 5개 확률/분기 로직에 대해 각각 최소 1개 이상의 테스트가 통과한다.
...
- 완료 조건:
  - [x] 상세 화면에서 신청 버튼 클릭 시 신청이 완료되고 버튼이 "신청 완료" 표시로 바뀐다.
  - [x] 이미 신청했거나 기간이 종료된 프로모션은 신청 버튼이 비활성화되고 안내 문구가 표시된다.
...
- 선행: BE-3, BE-4, FE-2
- 완료 조건:
  - [x] 관리자 계정으로 로그인했을 때만 등록/수정 화면 진입이 가능하다.
  - [x] 폼으로 프로모션을 등록/수정하면 목록 화면에 즉시 반영된다.

### FE-10. 공통 네비게이션 및 반응형 마감
- 상단 네비게이션(나의 신청 목록 / 펫 보기 축소판 버튼)을 공통 레이아웃(`frontend/src/components/layout/Layout.jsx`)으로 두고 좁은 화면에서는 아이콘 위주, 넓은 화면에서는 가로 나열되도록 CSS를 정리한다(`8-wireframe.md` 3번 네비게이션 규칙).
- "펫 보기"는 화면 이동이 아니라 팝업 토글로 동작한다(사용자 피드백 반영: 어느 화면에서든 우상단 버튼으로 펫을 열고/닫을 수 있어야 함). `/pet` 라우트 자체는 직접 진입용으로 유지.
- 전체 페이지(목록/상세/나의신청/펫)를 실제 모바일 폭과 데스크톱 폭에서 확인해 레이아웃 깨짐을 마감한다.
- 선행: FE-4, FE-7
- 완료 조건:
  - [x] 모든 화면에서 상단 네비게이션으로 목록/나의 신청 목록 이동, 펫 보기 팝업 열기/닫기가 가능하다.
  - [x] 좁은 화면과 넓은 화면 각각에서 레이아웃이 깨지지 않고 문서에 정의된 배치대로 보인다.
