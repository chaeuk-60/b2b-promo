...
- **JWT**: PRD 5.1절 그대로 — 액세스 토큰 15분 만료(Bearer 헤더), 리프레시 토큰 14일 만료(httpOnly 쿠키). 리프레시 토큰은 서버 저장 테이블 없이 스테이트리스로 시작(로그아웃 즉시 무효화는 범위 밖, PRD 10절 리스크로 이미 인지됨). 이 결정을 뒤집어 블랙리스트 테이블/Redis를 미리 만들지 않는다.
